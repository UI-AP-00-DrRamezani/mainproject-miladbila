...
public class Shoes extends ClothingProducts {
    ...
    public void addDiscount(int capacity, String validityDuration) {
        this.getDiscountList().add(new Discount(capacity, 35, validityDuration, false));
    }

    @Override
    public void allTimeDiscount(int capacity, String validityDuration) {
        this.getDiscountList().add(new Discount(capacity, 35, validityDuration, true));
    }

    @Override
    public void makeDiscountCode(Discount discount) {
        if (discount.getCode() == null)
            discount.setCode("sho" + (int) (1000 + (Math.random() * 9999)));
    }
}
